...
internal abstract class Hentai2ReadBaseParser(
        ...
        private fun buildListUrl(page: Int, order: SortOrder, filter: MangaListFilter): String {
                return buildString {
                        ...
                        if (filter.query.isNullOrEmpty()) {
                                when (order) {
                                        SortOrder.POPULARITY -> append("/hentai-list/all/any/all/most-popular/$page/")
                                        SortOrder.RATING -> append("/hentai-list/all/any/all/top-rating/$page/")
                                        else -> append("/hentai-list/all/all/all/all/$page/")
                                }
                        } else {
                                val searchStr = filter.query.urlEncoded()
                                when (order) {
                                        SortOrder.POPULARITY -> append("/hentai-list/search/$searchStr/all/most-popular/$page/")
                                        SortOrder.RATING -> append("/hentai-list/search/$searchStr/all/top-rating/$page/")
                                        else -> append("/hentai-list/search/$searchStr/all/name-az/$page/")
                                }
                        }
                }
        }
        ...
        private fun parseMangaList(doc: Document): List<Manga> {
                ...
                return doc.select("div.book-grid-item-container").mapNotNull { div ->
                        val a = div.selectFirst("div.overlay-title a[href]") ?: return@mapNotNull null
